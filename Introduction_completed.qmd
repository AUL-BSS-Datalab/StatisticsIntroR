...
```{r Load data}
e_votes <- read_excel("data/folketingsvalg.xlsx", sheet = "votes")
e_seats <- read_excel("data/folketingsvalg.xlsx", sheet = "seats")
```
...
e_small <- select(e_votes, "year", "votes_s", "votes_k", "votes_df", "votes_v", "votes_el", "votes_la", "votes_alt", "votes_rv", "votes_nb", "votes_sf")
...
e_small <- pivot_longer(e_small, cols = 2:11, names_to = "party", values_to = "votes")

#Finds the maximum value in the column
max(e_small$votes, na.rm = TRUE)

#Finds the minimum value in the column
min(e_small$votes, na.rm = TRUE)

#Displays an overview of min, max, median, mean, etc.
summary(e_small$votes)
...
e_data <- left_join(e_votes, e_seats, by = "year")
...
e_venstre$votes_pct <- round(e_venstre$votes_pct, 1)

e_venstre$seats_pct <- round(e_venstre$seats_pct, 1)
